...
export default async function handler(req, res) {
  ...
  try {
    ...
    const adminHtml = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background-color: #fafafa; border: 1px solid #e0e0e0; border-radius: 8px;">
        ...
        <div style="padding: 25px;">
          <h2 style="color: #333; margin-bottom: 20px; font-size: 20px;">New Banquet Booking Request</h2>
          <div style="background-color: #fff; padding: 20px; border: 1px solid #e0e0e0; border-radius: 6px;">
            ...
            <p style="margin: 10px 0; color: #444;"><strong>Event Type:</strong> ${bookingData.eventType}</p>
            <p style="margin: 10px 0; color: #444;"><strong>Number of Guests:</strong> ${bookingData.guests}</p>
            <p style="margin: 10px 0; color: #444;"><strong>Event Date:</strong> ${eventDate}</p>
            ${bookingData.specialRequests ? `<p style="margin: 10px 0; color: #444;"><strong>Special Requests:</strong> ${bookingData.specialRequests}</p>` : ''}
          </div>
        </div>
        ${emailFooter}
      </div>
      ...
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background-color: #fafafa; border: 1px solid #e0e0e0; border-radius: 8px;">
        ...
        <div style="padding: 25px;">
          ...
          <p style="color: #555; line-height: 1.6;">
            Thank you for choosing ${HOTEL_INFO.name} for your ${bookingData.eventType}! Our events team will contact you shortly.
          </p>
          
          <div style="background-color: #fff; padding: 15px; border: 1px solid #e0e0e0; border-radius: 6px; margin: 20px 0;">
            <p style="margin: 8px 0; color: #444;"><strong>Event Type:</strong> ${bookingData.eventType}</p>
            <p style="margin: 8px 0; color: #444;"><strong>Event Date:</strong> ${eventDate}</p>
            <p style="margin: 8px 0; color: #444;"><strong>Number of Guests:</strong> ${bookingData.guests}</p>
          </div>
    ...
    const adminResult = await sendEmail({
      ...
      subject: `Banquet Booking: ${bookingData.eventType} - ${bookingData.name}`,
      html: adminHtml
    });
    ...
    return res.status(200).json({ success: true, message: 'Banquet booking request received successfully.' });

  } catch (error) {
    console.error('Banquet booking error:', error.message);
