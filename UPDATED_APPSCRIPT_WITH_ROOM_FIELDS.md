# Updated Google Apps Script with Room Type and Occupancy Type Fields
...
function processFormData(sheet, data, formType, id, timestamp) {
  ...
  if (formType === "contact") {
    rowData = [
      ...
      data.roomType || "",                   // Room Type - New field
      data.occupancyType || "",              // Occupancy Type - New field
...
function createAndFormatHeaders(sheet, formType) {
  ...
  if (formType === "contact") {
    headers = [
      ...
      "Enquiry Type", 
      "Event Type",
      "Room Type",        // Added new field
      "Occupancy Type",   // Added new field
      "Message"
    ];
  } 
  else if (formType === "room") {
...
1. Added Room Type and Occupancy Type fields to the contact form data processing:
   ```javascript
   if (formType === "contact") {
     rowData = [
       // ...other fields
...
2. Added Room Type and Occupancy Type to the contact form headers:
   ```javascript
   if (formType === "contact") {
     headers = [
       // ...other headers
...
3. Added column width configuration for the new fields:
   ```javascript
   else if (formType === "contact") {
     sheet.setColumnWidth(5, 140);  // Contact Number
     sheet.setColumnWidth(6, 140);  // Enquiry Type
...
5. Test the form submission to verify the Room Type and Occupancy Type fields are now being collected
